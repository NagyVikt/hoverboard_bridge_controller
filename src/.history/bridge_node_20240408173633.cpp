...
private:
    ...
    double damping_factor = 0.8; // Adjust this value to control the damping effect
    ...
    void logInfo(const std::string& message) {
        ...
        log_stream << std::put_time(std::localtime(&now_time_t), "%Y-%m-%d %H:%M:%S");
        log_stream << '.' << std::setfill('0') << std::setw(3) << now_ms.count();
        log_stream << std::setfill('0') << std::setw(3) << now_ns.count();
        log_stream << ": " << message;
